...
import com.longoka.games.lexikongo.LingalaWordSearchService;
...
public final class LingalaNounWordSearchJsonExportTool {

 private LingalaNounWordSearchJsonExportTool() {
 }

 public static void main(String[] args) throws Exception {
  System.out.println("== Export JSON : Mots mêlés Lingala (noms) ==");
  ...
  String outputPath = "target/lingala-nouns-wordsearch-book.v1.json";

  LingalaWordSearchService service = new LingalaWordSearchService();
  ...
  for (int i = 0; i < puzzleCount; i++) {
   ...
   WordSearchPuzzle puzzle = service.generateRandomLingalaWordSearch(
     rows,
     cols,
     maxWords,
     meaningLanguageCode);
  ...
  pack.language = "ln";
  pack.packId = "ln-nouns-auto-" + LocalDate.now();
  pack.title = "Lingala – mots mêlés (noms)";
  ...
  System.out.println("✅ Export JSON (Lingala noms) terminé : " + out.toAbsolutePath());
 }
