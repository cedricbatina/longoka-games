...
(function () {
    ...
    // Même logique que les livres FromPack : LG.DOCUMENT_PAGE_PROFILE (défaut longokaA5).
    var pageCfg = LG.getPageSpec();
    var trim = String(book.trimSize || "").toLowerCase();
    if (trim === "6x9in" || trim === "6x9" || trim === "trade") {
        pageCfg = LG.PAGE;
    }
    ...
    // Title block
    var titleFrame = addFrame(page, LG.pageBox(page, 58, 22, 104, 132), compactTitle(titleText), LG.STYLES.p.bookTitle);
    titleFrame.paragraphs[0].justification = Justification.LEFT_ALIGN;

    if (subText) {
        var sub = addFrame(page, LG.pageBox(page, 108, 22, 120, 132), String(subText), LG.STYLES.p.bookSubtitle);
    ...
    var footer = addFrame(page, LG.pageBox(page, 192, 22, 205, 132), badges.join("  |  "), LG.STYLES.p.footer);
    ...
    function drawCoverBackground(doc, page, pack, isPremium) {
        ...
        bg.geometricBounds = LG.pageBox(page, -10, -10, 230, 160); // generous bleed (repère maquette ref.)
        ...
        band.geometricBounds = LG.pageBox(page, 0, 0, 36, 160);
        ...
        chip.geometricBounds = LG.pageBox(page, 22, 22, 30, 78);
    ...
    function tryPlaceCoverImage(doc, page, imgPath) {
        ...
        frame.geometricBounds = LG.pageBox(page, 126, 22, 186, 132);
